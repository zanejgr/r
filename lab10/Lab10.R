...
mymaxlik = function(lfun, x, param, ...) {
  ...
  ifelse(i - 3 >= 1 &
           i + 2 <= np,
         slope <-
           (y[(i - 2):(i + 2)] - y[(i - 3):(i + 1)]) / (param[(i - 2):(i + 2)] - param[(i -
                                                                                          3):(i + 1)]),
         slope <- "NA")
  return(list(
    i = i,
...
mymld = function(lfun, x, param, delta = 0.0001) {
  # param = parameter values, delta=accuracy, x=data
  ...
  layout(matrix(
    ...
    nc = 2,
    byrow = TRUE
  ))# divide plotting space for two graphs
  plot(
    param,
    y,
    col = "Blue",
  ...
  )# plot log lik Vs parameter values
  ...
  plot(
    ...
    xlab = expression(theta),
    las = 2
  ) # construct new plot for refined estimate
...
logbin2 = function(theta) {
  log(dbinom(1, prob = theta, size = 2)) + log(dbinom(7, prob = theta, size =
                                                        8))
}

mymaxlikg = function(lfun = "logbin2", theta) {
  # default log lik is a combination bin
  nth = length(theta)  # nu. of valuse used in theta
  thmat = matrix(theta,
                 nr = nth,
                 nc = 1,
                 byrow = TRUE) # Matrix of theta
...
mymlbeta = function(x, alpha, beta, ...) {
  ...
  zz = c()    ## initialize a new vector
  lfun = function(x, a, b)
    log(dbeta(x, shape1 = a, shape2 = b))   # log lik for beta
  for (j in 1:nb) {
...
for (i in 1:9) {
  w = mymlbeta(
    x = sample(sam, nsam, replace = TRUE),
    alpha = seq(0.1, 20, length = 100),
    beta = seq(0.1, 20, length = 100),
    lwd = 2,
    labcex = 1,
    col = "steelblue"
  )
...
logbinpois = function(theta1, theta2)
  log(dbinom(5, size = 10, prob = theta1)) + log(dbinom(7, size = 8, prob =
                                                          theta1)) + log(dpois(3, lambda = theta2))
...
maxlikg2(
  theta1 = seq(0, 1, length = 1000),
  theta2 = seq(0, 10, length = 1000),
  nlevels = 20
)
